...
const EditableCell: React.FC<EditableCellProps> = ({ value: initialValue, row, column, updateRecord, editable }) => {
  console.log('EditableCell props:', { value: initialValue, row, column });
...
export const FinancialRecordList = () => {
  ...
  // Memoize the updateCellRecord function to avoid unnecessary re-creations
  const updateCellRecord = useCallback((rowIndex: number, columnId: string, value: string | number) => {
    const id = records[rowIndex]?._id;
    if (id) {
      updateRecord(id, { ...records[rowIndex], [columnId]: value });
    }
  }, [records, updateRecord]); // Depend only on records and updateRecord

  // Memoizing columns to prevent unnecessary re-creations
  ...
  ], [updateCellRecord]); // Only depend on updateCellRecord
